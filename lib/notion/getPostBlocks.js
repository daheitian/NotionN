...
/**
 * Flatten notion-client >= 7.0 nested recordMap entries back to the classic
 * flat shape the rest of this fork expects.
 * @param {object} recordMap
 * @returns {object}
 */
function normalizeRecordMap(recordMap) {
  if (!recordMap) return recordMap
  for (const mapName of ['block', 'collection', 'collection_view', 'notion_user']) {
    const m = recordMap[mapName]
    if (!m) continue
    for (const key of Object.keys(m)) {
      ...
      if (
        ...
        m[key] = {
          ...
          spaceId: entry.spaceId ?? entry.value.value.space_id
        }
      }
    }
  }
...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      let pageData = await api.getPage(id)
      const end = new Date().getTime()
      console.log('[API<<--响应]', `耗时:${end - start}ms - from:${from}`)

      // notion-client >= 7.0 nests every recordMap entry as
      //   { spaceId, value: { role, value: <realData> } }
      // while this fork (and the react-notion-x data layer) expect the classic
      // flat shape { role, value: <realData> }. Unwrap one level so downstream
      // code that reads block[id].value.type etc. keeps working.
      pageData = normalizeRecordMap(pageData)
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    console.log('[API<<--响应]')
    const _normalized = normalizeRecordMap({ block: pageChunk?.recordMap?.block || {} })
    fetchedBlocks = Object.assign({}, fetchedBlocks, _normalized.block)
  }
