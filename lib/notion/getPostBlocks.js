...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      // Normalize common shapes so downstream code can always read `.block`.
      if (pageData && pageData.recordMap) {
        return pageData.recordMap
      }

      // Some versions may return recordMap wrapped differently
      if (pageData && pageData.recordMapWithRoles && pageData.recordMapWithRoles.recordMap) {
        return pageData.recordMapWithRoles.recordMap
      }

      // If pageData already looks like a recordMap (has .block), return as-is.
      if (pageData && pageData.block) return pageData
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    // Normalize pageChunk shape to extract blocks safely
    const recordMap =
      pageChunk?.recordMap ||
      pageChunk?.recordMapWithRoles?.recordMap ||
      (pageChunk?.block ? pageChunk : null)
