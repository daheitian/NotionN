...
/**
 * notion-client >= 7 把每条 recordMap 条目的真实数据又包了一层：
 *   block[id] = { spaceId, value: { role, value: <真实数据> } }
 * 而旧代码（以及 NotionNext 各处的读取逻辑）都假设
 *   block[id].value 就是真实数据本身（直接读 .type / .properties 等）。
 * 这里把嵌套结构拍平回经典形状 { role, value }，保证下游代码无需改动。
 * 对已经是扁平结构的响应，此函数是无操作的（安全）。
 *
 * 注意：拍平结果里绝不能出现 undefined 字段。notion_user 等记录没有 space_id，
 * 若写成 spaceId: undefined，Next.js 序列化 getStaticProps 返回值时会直接报
 * "undefined cannot be serialized as JSON" 并让整个构建失败；role 缺失同理补 null。
 */
function flattenRecordMap(recordMap) {
  if (!recordMap || typeof recordMap !== 'object') return recordMap
  const maps = ['block', 'collection', 'collection_view', 'notion_user']
  for (const name of maps) {
    const m = recordMap[name]
...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      // NOTE: Notion API / notion-client 在不同版本返回的数据形状不一致。
      // notion-client >= 7 把每条 recordMap 条目的真实数据又包了一层
      // `.value.value`，这里统一拍平，使下游读取 `block[id].value` 时拿到的
      // 就是真实数据（含 .type / .properties 等），从而兼容 8.x 与旧版。
      const recordMap =
        pageData && pageData.recordMap ? pageData.recordMap : pageData
      flattenRecordMap(recordMap)
      return recordMap
    } catch (e) {
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    const chunkBlock = pageChunk?.recordMap?.block
    // 同样拍平 notion-client >= 7 的嵌套结构
    if (chunkBlock) {
      flattenRecordMap({ block: chunkBlock })
    }
